...
[`../README.md`](../README.md)) exposes **56 `pf_*` tools**. Every tool maps to
...
| `pf_create_work_item` | Create a work item. Runs F3 dedup; `force_create` bypasses on a soft-conflict. **`requires_human_session` has THREE states, not two (aihub#411 T2-9):** `true`, `false`, and **omitted**. Omitting it stores `NULL`, which is not a default of `false` — the wi lands in the ready queue's `unclassified[]` segment rather than `items[]`, so it is never offered for unattended dispatch and `pf_list_work_items`' `ready_only` filter does not return it. `NULL` is **not permanent**: the FIRST `pf_claim_work_item` on such a wi resolves it to `true` from a server default, writes that back and records a `wi_classification_resolved` event — so a wi that has ever been claimed cannot still be unclassified. Send `false` explicitly for a wi an agent may take unattended. **`workflow_mode` (aihub#720) is the EXPLICIT composition-mode selector:** `legacy` (scenario step graph, the default when steps are omitted and the field is absent), `pending` (create uncomposed — the claim refuses with 409 `COMPOSE_PENDING` until a first generation is pinned via `pf_update_workflow`), `db` (pins generation 1 in the same create and REQUIRES `steps`, which the create tools do not publish — create, then `pf_update_workflow`). A bad or contradictory combination answers 400 `COMPOSE_FAILED` with a machine-readable `details.reason`, never a silent legacy fall-back. See `docs/workflow-v2/07-wi-composition.md`. |
...
| `pf_complete_attempt` | End the current attempt: `wrapped` (success), `failed`, or `paused`. Wrapping requires `derived`, the per-finding disposition list for what the attempt noticed but did not fix (`[]` allowed, omission refused). `note` records the closing note in the same call, before the state file is deleted. |
...
| `pf_recall` | Recall memories with filters. Item `content` is truncated to 800 runes; such items carry `content_truncated: true` and `content_full_len` (full rune length) — read the rest with `pf_get_memory` (aihub#269). **`request_adjusted` (aihub#314):** present only when the server changed a parameter you sent — a list of `{param, requested, applied}`. Today the one case is `top_k`, which is capped at 200 (and replaced with the default 20 when negative); an absent key means nothing was adjusted. **Ranking (aihub#192, shipped):** pgvector semantic recall is live, not pending. `Recall` is a router — it takes the vector path when an embedding provider is configured, `query` is non-empty, no `work_item_id` filter is set (a wi-scoped recall is deterministic, not semantic) and the type filter is empty or names at least one embeddable type; the text/tag path otherwise, with both halves merged when a request spans them (aihub#270). **The sort key differs per path** and is not the reference time except on one: cosine similarity bucketed to 0.01 on the vector path (aihub#311 — cosine must dominate, because a returned page's cosines sit close enough together that any strength weight can flip a near-tie. That read "the embedding model packs a result set into a ~0.04-wide band" until aihub#677: the narrow band is a property of the PAGE not the candidate set (aihub#646/#647), and ~0.04 was the fingerprint of the serving defect aihub#648 found, not of this model — the repaired page band reads median 0.1281 (aihub#650)), `ts_rank` on the lexical text branch, and `GREATEST(last_activated_at, created_at) DESC, id DESC` only on the text branch with no query. What every path *does* share is `GREATEST(last_activated_at, created_at)` as the **reference time inside the strength-decay expression** — never a `NULLS LAST` tier (aihub#236). There is **no** similarity floor by default; `similarity_threshold` sets one and gates the embeddable half only (aihub#148). Similarity values are not comparable across different queries, and the vector path does not paginate. **Lexical section (aihub#360):** every response to a request carrying `query` also holds a parallel top-level `lexical` section — verbatim-substring retrieval (every whitespace token, case-insensitive, must appear in a row's content), whichever path served `items`. Its hits carry no similarity, and an empty match set is an explicit `total: 0`, never an absent field. It exists because the single-vector unchunked index structurally misses excerpt queries: one unchunked vector per row cannot be asked for a verbatim excerpt of that row. Measured 2026-09-14 in the repaired embedding space (aihub#660, over the 44 queries aihub#367 froze at ce13215): 12 of 42 still miss at @10, five of them out of reach of any ranking — so a semantic miss is not evidence of absence; judge existence by the lexical section. 🔴 aihub#367's own 2026-09-06 numbers (recall@1 0/42, production-shape recall@10 33.3% against a 14.1% random baseline) are WITHDRAWN as evidence by aihub#677: they were taken through the aihub#648 serving defect, and re-embedding the corpus after aihub#650 repaired it moved that identical query set to 20/26/30 at @1/@5/@10. **Query prefix (aihub#669):** on the vector path the query is embedded with the model's instruct prefix while stored rows are not, which shifts every absolute cosine DOWN on any server carrying it — recalibrate a `similarity_threshold` you set against a server without it. The `lexical` section still matches the query you sent, verbatim. See "A recall query is embedded with an instruct prefix" below. Authority: `internal/domain/memory.go` (`recallRouted`), `internal/domain/memory_vector.go` (`RecallWithVector`) and `internal/domain/memory_lexical.go` (`recallLexical`). |
...
## A recall query is embedded with an instruct prefix; documents are not (aihub#669)

**On the semantic path of `pf_recall` and of `pf_list_work_items(query=)`, the
query text is embedded with the model's instruct prefix in front of it, while
stored memories and work items are embedded bare. That asymmetry is the model's
designed usage, not a defect.** Qwen3-Embedding ships `prompts.query` (the
template below) and `prompts.document = ""` in its own
`config_sentence_transformers.json`, so the prefix belongs on the question and
not on the corpus. Nothing was re-embedded to adopt it and nothing would need
re-embedding to drop it.
...
91 bytes, one newline, and no separator after `Query:` — the colon fuses with
the query's first character, because sentence-transformers concatenates prompt
and text. Authority: `internal/domain/embed_input.go` (`QueryEmbedPrefix`,
`QueryEmbedInput`), applied at exactly two call sites,
`internal/domain/memory_vector.go` and `internal/domain/wi_vector.go`.

Measured on production against 44 frozen queries, criterion unchanged, on the
vector space `aihub#650` repaired (`aihub#660`, 2026-09-14):

| | without the prefix | with it |
|---|---|---|
| recall @1 / @5 / @10 | 20 / 26 / 30 | **22 / 29 / 30** |
| garbage-string top-1 cosine | 0.6300 | **0.3290** |
| real queries scoring below that garbage string | 17/36 | **0/36** |
| lexical-section target hits | 2 | **2** |

Three consequences worth knowing before you act on a similarity number:
...
1. **The prefix lowers EVERY similarity.** It does not raise the score of a
   ...
   -0.0378, the garbage control -0.3010). The real-query top-1 median moved
   0.6408 to 0.6030, so a `similarity_threshold` or `min_similarity` calibrated
   against a server without this behaviour is reading a shifted scale. The
   change merged 2026-09-14 and reaches a given server at its next DEPLOY, not
   at the merge, so recalibrate against live numbers rather than against notes.
2. **The margin is thin.** The lowest real query in that set clears the garbage
   ...
   most. If a relative threshold or a relevance filter is ever proposed, that
   number is its ceiling, not a comfortable gap. Nothing watches it today.
3. **The lexical section is unaffected, by design.** It matches the query you
   sent, verbatim, and never sees the prefix. That is the whole reason the
   prefix is applied at the embed call rather than to the request: prefixing the
   request field would make the section search for rows containing
   `Instruct: Given a web search query...` and return zero every time
   (`aihub#660` measured exactly that: target hits 2 to 0, `tokens_used` mean
   10.05 to 15.64).

The `emb_model` stamp is unchanged by this, which means a query-side prefix is
not distinguishable from the data. `aihub#661` owns whether the stamp should
carry pipeline identity.
...
| `pf_wrap` | Push + PR + `complete_attempt(wrapped)` + delete state file. Requires `derived`, the per-finding disposition list for what the attempt noticed but did not fix (`[]` allowed, omission refused before anything is pushed). Idempotent only when a PR already covers local HEAD; see `pr_action` in the response. `note` records the closing note in the same call. |

## Skills (4) - `internal/mcp/tools_skills.go`

The skill-registry READ surface (aihub#720): what a workflow composer needs
before it can name exact `skill_id` + `skill_version` pairs in a create-with-
steps proposal or a workflow revision. Visibility is the registry's own
(aihub#708 Batch 1A): an unknown skill and an inaccessible one answer the same
`NOT_FOUND`, lists never offer a version the caller cannot read, and an exact
version request never falls back to latest. The registry's write and share
surface (create, publish, share, revoke, visibility) is deliberately NOT
published as MCP tools.

| tool | purpose |
|---|---|
| `pf_list_skills` | List skills and their latest ACCESSIBLE version metadata, scoped to the caller's registry view (owned / shared with one of the caller's projects / public). A skill with no version the caller can read is not listed. `owner`, `cursor`, `limit` paginate. |
| `pf_get_skill` | Read one skill's identity and the caller's latest accessible version summary. No content — no bundle, no contract. |
| `pf_list_skill_versions` | List the versions of one skill accessible to the caller — summaries only (version, visibility, digest, author). |
| `pf_get_skill_version` | Read one EXACT accessible version including its immutable bundle and runtime contract. `version` is required and never falls back to latest; a version the caller cannot read answers `NOT_FOUND`. |
