...
BASELINE = {
    ...
    "fragments/memory-conventions.md": (6457,
        ...
        "that look big'. It is deliberately NOT resident: the resident payload sits hard "
        "against its two-sided gate (live figures come from "
        "tests/using-polyforge-payload.test.sh and are deliberately NOT copied here -- "
        "this sentence used to read '8,452 of a 8,497 gate' and aihub#338 invalidated it "
        "the moment it changed the payload), and the rule is unenforceable in the "
        "abstract anyway "
