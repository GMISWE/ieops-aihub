...
| `pf_list_work_items` | List work items with filters (status, kind, label, milestone, user, source, ...). `sort=created_at\|closed_at` + `order=desc\|asc` control the ordering; `sort=closed_at` returns only closed items. **Response is projected (aihub#278):** items omit `content` (this endpoint never serves bodies — read one with `pf_get_work_item`) and `seq` (it is the part of `slug` after `#`), omit `scenario` when it is the universal `coding`, and omit `milestone` / `parent_work_item_id` / `closed_at` / `current_attempt_id` / `external_share_type` / `external_share_key` when those are null. **Absent therefore means null for those six, and nothing is omitted whose value you could not have reconstructed** — in particular `requires_human_session` is always present, because its null is a third state (unclassified) and not "none". |
