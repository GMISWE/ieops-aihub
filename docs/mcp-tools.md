...
| `pf_list_work_items` | List work items with filters (status, kind, label, milestone, user, source, ...). `sort=created_at\|closed_at` + `order=desc\|asc` control the ordering; `sort=closed_at` returns only closed items. **Response is projected (aihub#278):** items omit `content` (this endpoint never serves bodies — read one with `pf_get_work_item`), omit `scenario` when it is the universal `coding`, and omit `milestone` / `parent_work_item_id` / `closed_at` / `current_attempt_id` / `external_share_type` / `external_share_key` when those are null. **Absent means null for those six, and nothing is omitted whose value you could not have reconstructed.** Two fields are deliberately NOT omitted despite qualifying: `requires_human_session` (its null is a third state, *unclassified*, not "none") and `seq` (reconstructible from `slug`, but it is the field real callers reach for most). |
