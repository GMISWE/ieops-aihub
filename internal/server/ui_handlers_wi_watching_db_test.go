...
func seedWatchFixture(t *testing.T, pool *pgxpool.Pool) watchFixture {
	...
	for _, p := range []string{f.readable, f.hidden} {
		for _, q := range []string{
			...
			// Locks before attempts: resource_locks.owner_attempt_id is ON
			// DELETE RESTRICT, so a leftover lock would fail the run_attempts
			// delete with SQLSTATE 23001 (aihub#601, the class aihub#593
			// measured).
