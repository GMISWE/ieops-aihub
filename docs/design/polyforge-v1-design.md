...
换成「cosine 分桶为主键」，原因是**返回页**上的 cosine 挤在一条窄带里 ⇒ 任何
非平凡的第二项都能翻转这么小的差距。该 commit 明确排除了「调权重就行」这条路
（`0.9/0.1` 也不够）。上面这个公式的非相似度权重合计 0.4，比被删掉的 0.3 更大。

> ⚠️ **这句话原本写的是「0.6B 嵌入模型把一个【结果集】里的 cosine 压进约 0.04
> 宽的带内」，两处都已更正（aihub#646/#647/#677）。** ① 窄带是**返回页**的属性，
> 不是候选集的：修好后的候选集实测跨 15–30 个 0.01 桶（中位 23）。
> ⚠️ 另有一个 **22–54 桶**的读数常被一起引用，那是 aihub#646 在生产主机上读**全集**
> 得到的，**测于坏空间**（与本节其他 aihub#646 数字同批，`memory_vector.go` 把它们
> 整段标为 `HISTORICAL, do not reuse`）—— 它支持的只是「候选集远宽于返回页」这个
> **结构**结论，该结论不随管线变化；**别把那对数字当今天的桶数引用**。
> ② **`0.04` 是 aihub#648 那个 serving 缺陷的指纹，不是这个模型的属性**
> —— 生产 TEI 曾以双向注意力前向一个 causal checkpoint，页带宽因此被压掉约 2.8×；
> aihub#650 修好并全量重嵌后，同一个页带宽中位数从 `0.0428` 变成 `0.1281`。
> 下面那个比值用的是**本节自己那份 fixture 的带宽**，不是任何一个外来的数字，
> 理由见 `internal/domain/recall_recency_replay_test.go` 的 `replayCosineBand`。
...
**0.2709** 的 cosine 差 = 上面那份 fixture 整页带宽（**0.0555**，由它自己的 20 行
算出而不是写死）的 **4.88 倍**；要让 cosine 保持主导，w 必须 **< 0.081**，而文档
默认是 0.3。

**这个结论不依赖取哪一个带宽**，这才是它站得住的原因：比值在 `0.04`（已退役的
坏空间读数）下是 6.8×、在本 fixture 的 `0.0555` 下是 4.88×、在修好后的生产页带宽
`0.1281` 下是 2.11×；对应的 w 上界分别是 `0.060` / `0.081` / `0.169`。三者都远在
0.3 之下 ⇒ 换任何一个带宽，「w=0.3 让 recency 压过 cosine」都成立。

第三条依据（⚠️ **已被后续测量削弱，见本小节末尾的更正**）：aihub#367 那套召回
评测集的基线说明**瓶颈不在排序**。
...
| familyB | 18 | 0 | 2 | 2 |
| familyC | 6 | 0 | 0 | 0 |
| **合计（2026-09-06）** | **42** | **0/42 = 0.0%** | 4/42 = 9.5% | **8/42 = 19.0%** |

🔴 **更正（aihub#677）：上面这张表是在一个坏掉的 embedding serving 下测的，不能
再当作「现行属性」引用。** aihub#648 实测生产 TEI 以**双向注意力**前向一个 causal
checkpoint（对正确 causal 参考的余弦只有 0.139–0.348，17/17 条文档）；aihub#650
换到 `89-1.9.3` 并全量重嵌后，**同一套冻结 query、判据逐字不变**：
...
| familyB | 18 | 11 | 13 | 14 |
| familyC | 6 | 3 | 6 | 6 |
| **合计（2026-09-14）** | **42** | 20/42 = 47.6% | 26/42 = 61.9% | **30/42 = 71.4%** |

⇒ **「天花板只有 19%」这句话已经不成立，今天是 71.4%。** 第三条依据因此被**削弱
但没有归零**：仍有 12 条（28.6%）的正确答案不在 top-10，重排序对它们依然无能为力，
其中 5 条在冻结判据下**结构上不可能命中**（aihub#660 arm 1）。它不再能支撑「瓶颈
就是候选召回」这么强的话。

**但撤下 `recency_weight` 的结论不依赖这一条。** 前两条依据各自独立成立：recency
本来就已参与全部三条排序（`eff_strength` 自带 `exp(-days/stability)`），而闭式判据
在**三个带宽读数下全部**判 w=0.3 失败（见上：6.8× / 4.88× / 2.11×，w 上界
0.060 / 0.081 / 0.169）。要重开这个提案，需要新的论据，不能只指出第三条已过时。
