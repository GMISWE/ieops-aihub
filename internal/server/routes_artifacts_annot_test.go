...
// TestAnnotHTML_V2_OlderVersionAggregation verifies that still-open commits
// from earlier versions in the supersede chain are merged into the head
// page's annotation fragment with their reply/resolve forms targeting the
// SOURCE memory, while resolved older commits are excluded (aihub#125).
func TestAnnotHTML_V2_OlderVersionAggregation(t *testing.T) {
	openOld := CommitEntry{
		ID: "cm_old_open", AuthorDisplay: "Alice", Body: "still relevant feedback",
		CreatedAt: "2026-06-01T00:00:00Z",
		Anchor:    &CommitAnchor{Quote: "Overview text"},
	}
	resolvedOld := CommitEntry{
		ID: "cm_old_resolved", AuthorDisplay: "Bob", Body: "already handled",
		CreatedAt: "2026-06-01T00:00:00Z", Status: "resolved",
	}
	html := buildAnnotationHTML("mem_v2", annotSpecBody, nil, olderOpenCommits{
		MemID: "mem_v1", Label: "v1", Commits: []CommitEntry{openOld, resolvedOld},
	})

	if !strings.Contains(html, "still relevant feedback") {
		t.Fatalf("older open commit missing from fragment")
	}
	if strings.Contains(html, "already handled") {
		t.Errorf("older RESOLVED commit must not be inherited")
	}
	for _, marker := range []string{
		`action="/ui/artifacts/mem_v1/commit/cm_old_open/reply"`,
		`action="/ui/artifacts/mem_v1/commit/cm_old_open/resolve"`,
		`"source_mem_id":"mem_v1"`,
		`"version_label":"v1"`,
		`from v1`,
	} {
		if !strings.Contains(html, marker) {
			t.Errorf("missing %q in fragment; excerpt: %s", marker, excerptStr(html))
		}
	}
}

// TestHandleArtifactHTML_InheritedRespectsVisibility verifies that the
// inherited-annotation aggregation applies the same per-memory visibility
// policy as the head page: a PRIVATE older version's open comments must not
// surface to a viewer who is not its author (aihub#125 review fix).
func TestHandleArtifactHTML_InheritedRespectsVisibility(t *testing.T) {
	oldCommits := marshalCommits(t, []CommitEntry{{
		ID: "cm_secret", AuthorDisplay: "Carol", Body: "secret private feedback",
		CreatedAt: "2026-06-01T00:00:00Z",
		Anchor:    &CommitAnchor{Quote: "Overview"},
	}})
	head := &domain.Memory{
		ID: "mem_head", Project: "testproj", Type: "methodology.spec",
		Visibility: "project", AuthorUserID: "u_author",
		RenderedHTML: strptr(annotSpecBody),
	}
	oldPrivate := &domain.Memory{
		ID: "mem_old", Project: "testproj", Type: "methodology.spec",
		Visibility: "private", AuthorUserID: "u_carol",
		RenderedHTML: strptr(annotSpecBody), Commits: oldCommits,
	}

	prevLoad := loadMemoryFn
	loadMemoryFn = func(_ context.Context, _ *pgxpool.Pool, id string) (*domain.Memory, *domain.AihubError) {
		if id == "mem_old" {
			return oldPrivate, nil
		}
		return head, nil
	}
	prevChain := versionChainFn
	versionChainFn = func(_ context.Context, _ *pgxpool.Pool, _ string) ([]domain.MemoryVersionRef, error) {
		return []domain.MemoryVersionRef{
			{ID: "mem_old", CreatedAt: "2026-06-01T00:00:00Z", Status: "superseded"},
			{ID: "mem_head", CreatedAt: "2026-06-02T00:00:00Z", Status: "active", IsCurrent: true},
		}, nil
	}
	defer func() { loadMemoryFn = prevLoad; versionChainFn = prevChain }()

	render := func(u *UserContext) string {
		e := echo.New()
		req := httptest.NewRequest(http.MethodGet, "/ui/artifacts/mem_head/html", nil)
		rec := httptest.NewRecorder()
		c := e.NewContext(req, rec)
		c.SetPath("/ui/artifacts/:id/html")
		c.SetParamNames("id")
		c.SetParamValues("mem_head")
		setUser(c, u)
		if err := handleArtifactHTML(nil)(c); err != nil {
			e.HTTPErrorHandler(err, c)
		}
	...
	// A non-author project writer must NOT see the private version's comments.
	if body := render(writerUser("testproj")); strings.Contains(body, "secret private feedback") {
		t.Errorf("private older-version commit leaked to non-author viewer")
	}
	// An admin (visibility bypass) DOES see them.
	if body := render(adminUser()); !strings.Contains(body, "secret private feedback") {
		t.Errorf("admin should inherit the private older-version commit")
	}
}
