...
EXPECTED SKILL BEHAVIOR:
  ...
  Step 5 — Batch save learnings (recall-before-remember protocol):
    For each finding:
      ...
      if similarity > 0.85: pf_activate_memory + pf_remember(dedup_mode="merge", content=..., supersedes_memory_id=...)
      elif similarity > 0.65: pf_remember(dedup_mode="suggest", content=..., attrs={similar_to: ...})
      else: pf_remember(dedup_mode="off", content=...)
    NOTE: pf_remember takes `content`; there is no `body` param. Example:
      pf_remember(type="experience.pitfall", content="<learning text>", project="marketplace",
...
ASSERT MCP CALLS (in order):
  ...
  - pf_save_artifact called with type="methodology.retro" and structured_payload
...
ASSERT STATE:
  ...
  - Two artifacts created: type="methodology.retro" and type="methodology.wrap_summary"

NOTE: pf_remember requires type, visibility, and content.
...
uses `content` param for pf_remember; saves retro artifact (type="methodology.retro");
